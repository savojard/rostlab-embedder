#!/usr/bin/env python
"""
ProtT5 embedding generator (Hugging Face)

Adapted from ESM2 script to work with Rostlab/prot_t5_xl_uniref50 family.
...
Example (online):
  python prot_t5_embed_perseq.py input.fasta \
...
Offline:
  python prot_t5_embed_perseq.py input.fasta \
    --model-dir /models/Rostlab_prot_t5_xl_uniref50 \
    --tokenizer-dir /models/Rostlab_prot_t5_xl_uniref50 \
    --device cuda:0 --dtype bf16 \
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate ProtT5 embeddings (HF) with per-sequence .npz outputs."
    )
    ...
    p.add_argument(
        ...
        help="HF model id (e.g. 'Rostlab/prot_t5_xl_uniref50'). Ignored if --model-dir is set.",
    )
    p.add_argument(
        "--model-dir",
        default=None,
    ...
    p.add_argument(
        ...
        help="Optional model-internal chunk size (if supported) to reduce VRAM "
             "(ProtT5 usually does not expose this; will be ignored if unsupported).",
    )
...
def prepare_prott5_input(seq: str) -> str:
    """
    Prepare an amino-acid sequence string for ProtT5:
    - remove whitespace
    - uppercase
...
def embed_chunk(
    ...
    """
    Embed a single sequence chunk with ProtT5 -> returns (per_residue_embeddings [L, H]).
    """
    seq_str = prepare_prott5_input(seq_chunk)
    ...
    # For T5EncoderModel / AutoModel on ProtT5, last_hidden_state is [1, T, H]
...
def main():
    ...
    # Optional model-internal chunk size (ProtT5 usually doesn't support this;
    ...
    if args.chunk_size is not None:
        try:
            ...
            print(f"⚠️ Could not set model chunk size (likely unsupported for ProtT5): {e}")
